...
typedef enum { TL_RED, TL_YELLOW, TL_GREEN } TrafficLightState;
typedef enum { GATE_OPEN, GATE_CLOSED }       GateState;
...
void setup() {
    ...
    lcd.init();
    lcd.backlight();
    lcd.setCursor(0, 0);  lcd.print(" CENTRALIDADE DO");
    lcd.setCursor(0, 1);  lcd.print("    KILAMBA     ");
    lcd.setCursor(-4, 2); lcd.print(" SISTEMA LIGADO ");
...
void applyTrafficLightPhase() {
  tlTimer = millis();
  switch (tlPhase) {
      case 0: setTrafficLight(&tl1, TL_GREEN);  setTrafficLight(&tl2, TL_RED);    break;
      case 1: setTrafficLight(&tl1, TL_YELLOW); setTrafficLight(&tl2, TL_RED);    break;
      case 2: setTrafficLight(&tl1, TL_RED);    setTrafficLight(&tl2, TL_GREEN);  break;
      case 3: setTrafficLight(&tl1, TL_RED);    setTrafficLight(&tl2, TL_YELLOW); break;
  }
}
...
void sendDataPacket() {
    ...
    Serial.print(F("TL1:")); Serial.print(tlStateToChar(tl1.state));
    Serial.print(F(",TL2:")); Serial.print(tlStateToChar(tl2.state));
    Serial.print(F(",SL1:")); Serial.print(sl1.isOn ? 1 : 0);
    Serial.print(F(",SL2:")); Serial.print(sl2.isOn ? 1 : 0);
    Serial.print(F(",SL3:")); Serial.print(sl3.isOn ? 1 : 0);
    Serial.print(F(",SM:"));  Serial.print(smoke.rawValue);
    Serial.print(F(",FR:"));  Serial.print(fire.detected    ? 1 : 0);
    Serial.print(F(",WL:"));  Serial.print(waterLevel.distanceCm, 1);
    Serial.print(F(",RN:"));  Serial.print(rain.raining     ? 1 : 0);
    Serial.print(F(",GAI:")); Serial.print(gateA.entranceState == GATE_OPEN ? 'O' : 'C');
    Serial.print(F(",GAO:")); Serial.print(gateA.exitState     == GATE_OPEN ? 'O' : 'C');
    Serial.print(F(",GBI:")); Serial.print(gateB.entranceState == GATE_OPEN ? 'O' : 'C');
    Serial.print(F(",GBO:")); Serial.print(gateB.exitState     == GATE_OPEN ? 'O' : 'C');
    Serial.print(F(",TLE:")); Serial.print(tlEnabled ? 1 : 0);
    Serial.print(PACKET_END);
    Serial.println();
}

void handleSerialCommand(const char *cmd) {
    char c0 = cmd[0], c1 = cmd[1], c2 = cmd[2];
    ...
    // Luzes de Rua
    else if (c0=='L' && c1=='1' && c2=='O') setStreetlight(&sl1, true);
    ...
    // Portão A — Entrada
    else if (c0=='A' && c1=='I' && c2=='O') openGate(&servoAIn,  &gateA.entranceState, &gateA.entranceTimer);
